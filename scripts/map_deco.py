...
def add_scalebar(ax, km=50):
    """Barra de escala en km (convierte a grados de longitud en la latitud media)."""
    x0, x1 = ax.get_xlim(); y0, y1 = ax.get_ylim()
    latmid = (y0 + y1) / 2.0
    deg = km / (111.32 * np.cos(np.radians(latmid)))
    xa = x0 + (x1 - x0) * 0.055
    ya = y0 + (y1 - y0) * 0.055
    ax.plot([xa, xa + deg], [ya, ya], color="#2b2b2b", lw=2.6,
            solid_capstyle="butt", zorder=20)
    for xt in (xa, xa + deg):
        ax.plot([xt, xt], [ya, ya + (y1 - y0) * 0.012], color="#2b2b2b",
                lw=1.2, zorder=20)
    ax.text(xa + deg / 2, ya + (y1 - y0) * 0.02, f"{km} km", ha="center",
